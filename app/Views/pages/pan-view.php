...
<div class="page-header">
    <div>
        <h1 class="page-title">Todo's - Pan</h1>
    </div>
                ...
                <div class="row">
                    ...
                    <div class="col-sm-4 col-md-4 col-lg-4 col-xl-4">
                        ...
                        <select id="filterStatus" name="filterStatus" class="form-select" style="font-size: 14px;">
                            ...
                            <option value="1" class="text-success">pan found</option>
                            <option value="2" class="text-warning">insufficient information</option>
                            <option value="3" class="text-danger">pan not found</option>
                            <option value="4" class="text-danger">name mismatched</option>
                            <option value="5" class="text-danger">fake pan</option>
                        </select>
                    </div>
                    <div class="col-sm-4 col-md-4 col-lg-4 col-xl-4 border-white border-1 rounded-2" style="height: 100px;background-color:#fff9e6;">
                        <div class="d-flex flex-column mb-3 p-4 ">
                            ...
                            <p class="text-medium">Todo's for PAN</p>
                        </div>
                    </div>
                    <!-- <div class="col-sm-4 col-md-4 col-lg-4 col-xl-4">
                            ...
                            <tbody>
                                ...
                                ?>
                                        <tr>
                                            ...
                                            if ($record['is_pan'] == 1) {
                                            ?>
                                                <td><span class="text-success  text-medium">pan found</span></td>

                                            <?php } elseif ($record['is_pan'] == 2) { ?>

                                                <td><span class="text-warning text-medium">insufficient info</span></td>

                                            <?php } elseif ($record['is_pan'] == 3) { ?>

                                                <td><span class="text-danger text-medium">pan not found</span></td>


                                            <?php } elseif ($record['is_pan'] == 4) { ?>

                                                <td><span class="text-danger text-medium">name mismatched</span></td>

                                            <?php } elseif ($record['is_pan'] == 5) { ?>

                                                <td><span class="text-danger text-medium">fake pan</span></td>
                                            ...
                                            <td><span>
                                                    <a href="javascript:void(0)" onclick="todoPanModal(<?= $record['id'] ?>)"><i class="fa fa-arrow-right"></i></a>
                                                    <!-- <a href="<?php echo base_url("EmployeeController/panDashboard?id=$id"); ?>"><i class="fa fa-arrow-right"></i></a> -->
                                                </span>
                                            </td>
                                        </tr>
                                    <?php endforeach;
    ...
    $('#filter').on('change', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                var tableHtml = '<thead><tr><th>Id</th><th>Candidate Name</th><th>PAN Number</th><th>Case status</th><th>TAT left</th><th>Initiated on</th><th></th></tr></thead>';
                $.each(response.data, function(index, item) {
                    var fullName = item.first_name + ' ' + item.last_name;
                    if (item.is_pan == 1) {
                        item.is_pan = '<span class="text-success text-medium">pan found</span>'
                    } else if (item.is_pan == 2) {
                        item.is_pan = '<span class="text-warning text-medium">insufficient info</span>'
                    } else if (item.is_pan == 3) {
                        item.is_pan = '<span class="text-danger text-medium">pan not found</span>'
                    } else if (item.is_pan == 4) {
                        item.is_pan = '<span class="text-danger text-medium">name mismatched</span>'
                    } else if (item.is_pan == 5) {
                        item.is_pan = '<span class="text-danger text-medium">fake pan</span>'
                    }
